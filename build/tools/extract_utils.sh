...
function setup_vendor() {
    ...
    export XDROID_ROOT="$3"
    if [ ! -d "$XDROID_ROOT" ]; then
        echo "\$XDROID_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$XDROID_ROOT/$OUTDIR" ]; then
        mkdir -p "$XDROID_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$XDROID_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDBP="$XDROID_ROOT"/"$OUTDIR"/Android.bp
    export ANDROIDMK="$XDROID_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$XDROID_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$XDROID_ROOT"/prebuilts/tools-xdroid/common/smali/baksmali.jar
        export SMALIJAR="$XDROID_ROOT"/prebuilts/tools-xdroid/common/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$XDROID_ROOT"/prebuilts/tools-xdroid/${HOST}-x86/bin/vdexExtractor
    fi

    if [ -z "$CDEXCONVERTER" ]; then
        export CDEXCONVERTER="$XDROID_ROOT"/prebuilts/tools-xdroid/${HOST}-x86/bin/compact_dex_converter
    fi

    if [ -z "$PATCHELF" ]; then
        export PATCHELF="$XDROID_ROOT"/prebuilts/tools-xdroid/${HOST}-x86/bin/patchelf
    fi
    ...
    if [ ! -f "$XDROID_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$XDROID_TARGET" >/dev/null; then
...
function extract() {
    ...
    local OUTPUT_ROOT="$XDROID_ROOT"/"$OUTDIR"/proprietary
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            do
                ...
                if [ -a "$DUMPDIR"/"$PARTITION".new.dat ]; then
                    ...
                    python "$XDROID_ROOT"/vendor/xdroid/build/tools/sdat2img.py "$DUMPDIR"/"$PARTITION".transfer.list "$DUMPDIR"/"$PARTITION".new.dat "$DUMPDIR"/"$PARTITION".img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$XDROID_ROOT"/"$OUTDIR"/radio
